<template>
  <div class="invoice-list" ref="abc">
    ...
    <el-pagination v-if="invoiceList.length != 0" layout="prev, pager, next" :current-page="currentPage" :page-size="pageSize" @current-change="currentChange" :total="invoiceList.length">
    </el-pagination>
...
export default class Invoice extends Vue {
  show: Boolean = false
  userInfo = null
  invoiceInfoVisible: Boolean = false
  currentPage: number = 1
  invoiceList: any[] = []
  currentPageData: any[] = []
  loadingInfo: Boolean = false // 加载订单详情
  currentInvoiceProgressList: null // 当前订单详情
  pageSize: number = 5 // 每页数据条数

  currentChange (pageNum: number): void {
    const start: number = (pageNum - 1) * this.pageSize
